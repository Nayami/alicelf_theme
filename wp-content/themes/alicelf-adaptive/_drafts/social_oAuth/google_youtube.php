...
function aa_func_20162003112035()
{
	...
	if ( isset( $_GET[ 'code' ] ) ) {
		...
		if ( isset( $tokenInfo[ 'access_token' ] ) ) {
			...
			$_client    = new Client( [ 'verify' => false ] );
			$__request = $_client->request( 'GET','https://www.googleapis.com/youtube/v3/channels?part=id&mine=true&access_token='.$get_params[ 'access_token' ]);
			$info = json_decode( $__request->getBody(), true );
			echo "<pre>";
			print_r($info);
			echo "</pre>";
